document.addEventListener("DOMContentLoaded", () => {
    ...
    const minLength = parseInt(
        reqList.getAttribute("data-min-length") || "8",
        10,
    );
    const requireMixedCase =
        reqList.getAttribute("data-require-mixed-case") === "true";
    const requireLetters =
        reqList.getAttribute("data-require-letters") === "true";
    const requireNumbers =
        reqList.getAttribute("data-require-numbers") === "true";
    const requireSymbols =
        reqList.getAttribute("data-require-symbols") === "true";
    ...
    function checkRules() {
        ...
        const rules = {
            ...
            mixed:
                !requireMixedCase || (/[a-z]/.test(val) && /[A-Z]/.test(val)),
        ...
        updateItemStyle("req-length", rules.length);
        if (requireLetters) updateItemStyle("req-letters", rules.letters);
        if (requireMixedCase) updateItemStyle("req-mixed", rules.mixed);
        if (requireNumbers) updateItemStyle("req-numbers", rules.numbers);
        if (requireSymbols) updateItemStyle("req-symbols", rules.symbols);

        // El requisito de match
        updateItemStyle("req-match", rules.match);

        // Control de estado deshabilitado del botón de envío
        // Se mantiene deshabilitado si alguno de los campos de contraseña está vacío (o solo tiene espacios)
        const bothFilled = val.trim() !== "" && confirmVal.trim() !== "";
        if (submitBtn) {
    ...
    function updateItemStyle(elementId, isValid) {
